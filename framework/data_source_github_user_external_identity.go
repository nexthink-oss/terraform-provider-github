	...
	var query struct {
		Organization struct {
			SamlIdentityProvider struct {
				ExternalIdentities githubpkg.ExternalIdentities `graphql:"externalIdentities(first: 1, login:$username)"` // There should only ever be one external identity configured
			}
