...
func TestAccGithubRepositoryCollaborators(t *testing.T) {
	...
	config := githubpkg.Config{BaseURL: "https://api.github.com/", Owner: testOrganization, Token: testToken}
	...
	t.Run("creates collaborators without error", func(t *testing.T) {
		...
		conn := meta.(*githubpkg.Owner).V3Client()
		...
		t.Run("with an individual account", func(t *testing.T) {
			check := resource.ComposeTestCheckFunc(
				...
				func(state *terraform.State) error {
					owner := meta.(*githubpkg.Owner).Name()
