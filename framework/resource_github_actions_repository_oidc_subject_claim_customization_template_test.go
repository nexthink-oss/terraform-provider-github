...
func TestAccGithubActionsRepositoryOIDCSubjectClaimCustomizationTemplateResource(t *testing.T) {
...
// TestAccGithubActionsRepositoryOIDCSubjectClaimCustomizationTemplateResource_Framework tests the pure Framework provider
func TestAccGithubActionsRepositoryOIDCSubjectClaimCustomizationTemplateResource_Framework(t *testing.T) {
	randomID := acctest.RandStringFromCharSet(5, acctest.CharSetAlphaNum)

	t.Run("creates repository oidc subject claim customization template with pure Framework provider", func(t *testing.T) {
		config := fmt.Sprintf(`
		resource "github_repository" "test" {
			name = "tf-acc-test-%s"
			visibility = "private"
