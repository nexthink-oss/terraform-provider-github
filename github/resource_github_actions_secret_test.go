...
func TestAccGithubActionsSecretResource_basic(t *testing.T) {
	...
	t.Run("creates and updates secrets without error", func(t *testing.T) {
		...
		checks := map[string]resource.TestCheckFunc{
			"before": resource.ComposeTestCheckFunc(
				...
				resource.TestCheckResourceAttr(
					"github_actions_secret.plaintext_secret", "secret_name",
					"test_plaintext_secret",
				),
				resource.TestCheckResourceAttrSet(
					"github_actions_secret.plaintext_secret", "id",
				),
			),
...
func TestAccGithubActionsSecretResource_import(t *testing.T) {
	randomID := acctest.RandStringFromCharSet(5, acctest.CharSetAlphaNum)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("github_actions_secret.test", "secret_name", "test_secret"),
					resource.TestCheckResourceAttr("github_actions_secret.test", "repository", fmt.Sprintf("tf-acc-test-%s", randomID)),
			...
			},
			{
				...
				ImportStateId:           fmt.Sprintf("tf-acc-test-%s/test_secret", randomID),
...
func TestAccGithubActionsSecretResource_validation(t *testing.T) {
	randomID := acctest.RandStringFromCharSet(5, acctest.CharSetAlphaNum)
