...
func TestAccGithubRepositoryEnvironmentDeploymentPolicyImport(t *testing.T) {
	randomID := acctest.RandStringFromCharSet(5, acctest.CharSetAlphaNum)

	t.Run("imports a repository environment deployment policy", func(t *testing.T) {
		config := fmt.Sprintf(`
			data "github_user" "current" {
				username = ""
			}
...
func TestAccGithubRepositoryEnvironmentDeploymentPolicyConfigValidation(t *testing.T) {
	randomID := acctest.RandStringFromCharSet(5, acctest.CharSetAlphaNum)

	t.Run("validates that exactly one pattern type is specified", func(t *testing.T) {
		// Test with neither pattern specified
		configNeitherPattern := fmt.Sprintf(`
			resource "github_repository" "test" {
				name = "tf-acc-test-%s"
			}
		...
		// Test with both patterns specified
		configBothPatterns := fmt.Sprintf(`
			resource "github_repository" "test" {
				name = "tf-acc-test-%s"
			}
		...
		resource.Test(t, resource.TestCase{
			...
			Steps: []resource.TestStep{
				{
					Config:      configNeitherPattern,
					ExpectError: regexp.MustCompile("Exactly one of branch_pattern and tag_pattern must be specified"),
				},
				{
					Config:      configBothPatterns,
					ExpectError: regexp.MustCompile("Only one of branch_pattern and tag_pattern may be specified"),
				},
			},
		})
	})
...
func TestAccGithubRepositoryEnvironmentDeploymentPolicyPlanChecks(t *testing.T) {
	randomID := acctest.RandStringFromCharSet(5, acctest.CharSetAlphaNum)

	t.Run("validates plan modification behavior for pattern type changes", func(t *testing.T) {
		config1 := fmt.Sprintf(`
			data "github_user" "current" {
				username = ""
			}
				...
				Steps: []resource.TestStep{
					{
						...
						Check: resource.TestCheckResourceAttr(
							"github_repository_environment_deployment_policy.test", "branch_pattern", "main",
						),
					},
					{
						Config: config2,
						...
						Check: resource.TestCheckResourceAttr(
							"github_repository_environment_deployment_policy.test", "tag_pattern", "v*",
						),
					},
				},
			})
