...
func TestAccGithubCodespacesOrganizationSecretRepositories_Migration(t *testing.T) {
	const ORG_SECRET_NAME = "ORG_SECRET_NAME"
	randomID := acctest.RandStringFromCharSet(5, acctest.CharSetAlphaNum)
	secret_name, exists := os.LookupEnv(ORG_SECRET_NAME)
...
func TestAccGithubCodespacesOrganizationSecretRepositories_ImportBasic(t *testing.T) {
	const ORG_SECRET_NAME = "ORG_SECRET_NAME"
	randomID := acctest.RandStringFromCharSet(5, acctest.CharSetAlphaNum)
	secret_name, exists := os.LookupEnv(ORG_SECRET_NAME)
		...
		testCase := func(t *testing.T, mode string) {
			resource.Test(t, resource.TestCase{
				...
				Steps: []resource.TestStep{
					...
					{
						ResourceName:      "github_codespaces_organization_secret_repositories.org_secret_repos",
						ImportState:       true,
